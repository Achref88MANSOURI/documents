    ...
    for item in data.get("vulnerabilities", []):
        ...
        for cvss_version in ["cvssMetricV31", "cvssMetricV30"]:
            if cvss_version in metrics and metrics[cvss_version]:
                ...
                actual_severity = metrics[cvss_version][0].get("baseSeverity", "CRITICAL")
                break
...
def main():
    ...
    print(f"Covering {TOTAL_DAYS_BACK} days back in {len(windows)} windows")
    ...
    for i, (w_start, w_end) in enumerate(windows, 1):
        ...
        print(f"Window {i}/{len(windows)}: {start_str[:10]} to {end_str[:10]}")
        ...
        num_pages = (total + 99) // 100

        for page in range(1, num_pages):
            time.sleep(NVD_REQUEST_DELAY)
            page_data = nvd_fetch(start_str, end_str, start_index=page * 100)
            window_cves.extend(parse_nvd_response(page_data))
    ...
    ok = 0
    for i, c in enumerate(all_cves, 1):
        point_id = str(uuid.uuid5(NAMESPACE, c['id']))
        vector = get_embedding(c['document'])
        upsert_point(point_id, vector, c['metadata'])
        ok += 1
        if i % 100 == 0 or i == len(all_cves):
            print(f"  {i}/{len(all_cves)} ingested")

    print(f"\nDone. {ok} CVEs ingested.")
